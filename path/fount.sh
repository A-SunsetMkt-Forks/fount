...
AUTO_INSTALLED_BUN_FLAG="$INSTALLER_DATA_DIR/auto_installed_bun" # 最终使用 Bun
...
# 函数: 运行 Bun，考虑 glibc-runner 如果可用，以及 .glibc.sh 包装器
run_bun() {
	local bun_args=("$@")
	local bun_cmd="bun" # Default to bun
	...
	if [[ ($IN_TERMUX -eq 0 && -z "$(command -v bun)") || ($IN_TERMUX -eq 1 && ! -f ~/.bun/bin/bun.glibc.sh) ]]; then
		...
		if [[ $IN_TERMUX -eq 1 ]]; then
			...
			if [[ $bun_successfully_installed -eq 1 ]]; then
				...
				# Test Bun with glibc-runner if available (Post-install verification from master branch)
...
bun_upgrade() {
	...
	if [[ -z "$bun_version_before" ]]; then
		echo "Error: Could not determine current Bun version. Skipping upgrade." >&2
	else
		...
		if [[ $upgrade_exit_code -ne 0 && $IN_TERMUX -eq 1 ]]; then
			echo "Warning: Bun upgrade failed in Termux. Attempting to reinstall Bun." >&2
			rm -rf "$HOME/.bun"
			install_bun
			upgrade_exit_code=$? # 更新退出码
		fi
		local bun_version_after=$(run_bun --revision 2>&1)

		# 检查是否需要重新 patch bun (仅限 Termux)
		if [[ "$bun_version_before" != "$bun_version_after" && $IN_TERMUX -eq 1 ]]; then
			echo "Bun version changed, re-patching for Termux..." >&2
			patch_bun
		fi

		if [ $upgrade_exit_code -ne 0 ]; then
			echo "Warning: Bun upgrade may have failed. Please check for errors." >&2
		fi
	fi
}
...
if [[ ! -d "$FOUNT_DIR/node_modules" || ($# -gt 0 && $1 = 'init') ]]; then
	...
	# Bun 的 install 命令通常不需要额外的参数，它会根据 package.json 自动处理
	run_bun install
	# 不知为何部分环境下第一次跑铁定出错，先跑再说 (模仿 PowerShell 脚本的行为)
	run_bun run --config="$FOUNT_DIR/bunfig.toml" --install=force --prefer-latest "$FOUNT_DIR/src/server/index.mjs" "shutdown" 2>/dev/null || true
	set -e # 重新启用严格模式
...
if [[ $# -gt 0 ]]; then
	...
	remove)
		...
		# 从当前 PATH 环境变量中移除 fount 和 Bun 的路径
		...
		if [[ $IN_TERMUX -eq 1 ]]; then
			...
			for package in "${INSTALLED_PACMAN_PACKAGES_ARRAY[@]}"; do
				pacman -R --noconfirm "$package" || echo "Could not uninstall Termux Pacman package: $package"
			done
		fi
		...
		if [ -f "$AUTO_INSTALLED_BUN_FLAG" ]; then
			...
			if [ -d "$HOME/.bun" ]; then
				rm -rf "$HOME/.bun"
				echo "Bun installation directory $HOME/.bun removed."
			fi
			for profile_file in "${profile_files[@]}"; do
				if [ -f "$profile_file" ]; then
					if [ "$OS_TYPE" = "Darwin" ]; then
						sed -i '' '/\.bun/d' "$profile_file"
					else
						sed -i '/\.bun/d' "$profile_file"
					fi
					echo "Cleaned Bun PATH entries in $profile_file."
				fi
			done
