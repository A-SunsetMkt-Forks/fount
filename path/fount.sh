...
load_installed_packages() {
	...
	# [合并后] 保留 master 分支的环境变量功能，移除 pacman 相关逻辑
...
save_installed_packages() {
	...
	# [合并后] 移除 master 分支的 pacman 相关逻辑，只保留系统包列表
	# 将数组元素用分号连接起来，写入文件
	(
		IFS=';'
		echo "${INSTALLED_SYSTEM_PACKAGES_ARRAY[*]}"
...
# [合并后] 移除 master 分支的 run_deno 和 patch_deno 函数，它们是 glibc-runner 方案的专属部分
...
# ====================================================================
# [合并后] 完整保留 proot 分支的核心逻辑
# Termux 环境下的特殊处理 (Proot 逻辑) - 必须放在脚本的最前面
# ====================================================================
if [[ $IN_TERMUX -eq 1 && $IN_PROOT -eq 0 ]]; then
	...
	# 注意：这里传递的参数需要正确引用，以确保在 proot 环境中被正确解析
	# \"$@\" 会将所有参数作为一个字符串传递，然后在 proot 环境中再次解析
...
# 检查 Deno 安装
install_deno() {
	AUTO_INSTALLED_DENO_FLAG="$INSTALLER_DATA_DIR/auto_installed_deno" # 确保标记文件路径正确
	...
	if [[ -z "$(command -v deno)" ]]; then # 检查 Deno 是否已安装
...
deno_upgrade() {
	# 获取 Deno 版本信息
	local deno_version_before=$(deno -V 2>&1)
	...
	if [[ -z "$deno_version_before" ]]; then
		...
		local deno_version_after=$(deno -V 2>&1)
...
run() {
	...
	# Termux 环境下设置 LANG (即使在 proot 中，也可能需要确保 LANG 正确)
	...
	# [合并后] 移除 master 分支的 run_deno 调用，直接使用 deno
...
if [[ ! -d "$FOUNT_DIR/node_modules" || ($# -gt 0 && $1 = 'init') ]]; then
	...
	if [ $IN_DOCKER -eq 0 ] && [ $IN_TERMUX -eq 0 ]; then # 仅在非 Docker/Termux 环境创建快捷方式
...
if [[ $# -gt 0 ]]; then
	...
	remove)
		...
		# [合并后] 移除卸载 Termux Pacman 包的逻辑
		# if [[ $IN_TERMUX -eq 1 ]]; then ... fi
